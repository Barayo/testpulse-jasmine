...
describe('case tagging (real jasmine run)', () => {
  it('the real generated report contains the injected property, and an untagged spec does not', async () => {
    ...
    const untaggedTestcase = report.match(/<testcase[^>]*name="untagged test"[^>]*>[\s\S]*?<\/testcase>|<testcase[^>]*name="untagged test"[^>]*\/>/);
